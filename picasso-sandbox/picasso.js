...
function getRect(container, settings) {
  var containerRect = { x: 0, y: 0, width: 0, height: 0 };
  var logicalContainerRect = { x: 0, y: 0, width: 0, height: 0 };
...
function reduceSingleLayoutRect(logicalContainerRect, reducedRect, edgeBleed, c, settings) {
  var newReduceRect = index({}, reducedRect);
  var newEdgeBeed = index({}, edgeBleed);
...
function reduceLayoutRect(logicalContainerRect, components, hiddenComponents, settings) {
  ...
  var edgeBleed = { left: 0, right: 0, top: 0, bottom: 0 };
...
function boundingBox(rects) {
  ...
  var points = (_ref = []).concat.apply(_ref, toConsumableArray(rects.map(getRectVertices)));
  return pointsToRect(points);
}

function positionComponents(components, logicalContainerRect, reducedRect, containerRect) {
  var vRect = { x: reducedRect.x, y: reducedRect.y, width: reducedRect.width, height: reducedRect.height };
  var hRect = { x: reducedRect.x, y: reducedRect.y, width: reducedRect.width, height: reducedRect.height };
...
var accessors = {
  ...
  formatter: function formatter(data) {
    return data.formatter && data.formatter() || formatterRegistry('d3-number')('');
  }
};
  ...
  if (Array.isArray(dataConfig)) {
    ...
    if (dataConfig.groupBy || dataConfig.mapTo) {
      ...
      if (dataConfig.extract) {
        ...
        var sourceFields = [];
        (Array.isArray(dataConfig.extract) ? dataConfig.extract : [dataConfig.extract]).forEach(function (ex) {
          if (ex.field) {
        ...
        dataConfig.fields.forEach(function (id) {
          var f = source.field(id);
          if (f) {
            if (!extracted.fields) {
              extracted.fields = [];
            }
        ...
        if (f) {
          ...
          extracted.items = f.items().map(function (v) {
            return { value: valueFn(v), source: { field: dataConfig.field } };
          });
    ...
    if (cfg) {
      if (cfg.fields && cfg.fields.filter) {
        ...
        if (coll.fields.length !== filtered.length) {
          coll = index(coll, { fields: filtered });
        }
      }
    }
...
function create$1(options, dataset, deps) {
  // TODO Have some magic to handle and merge formatters from multiple sources

  if (options.source) {
    var match = dataset.findField(options.source, fieldFinder);
...
function builder(obj, dataset, deps) {
  var formatters = {};
  for (var f in obj) {
    if (Object.prototype.hasOwnProperty.call(obj, f)) {
      formatters[f] = create$1(obj[f], dataset, deps);
    }
  }
  return formatters;
}

function getOrCreateFormatter(v, formatters, dataset, deps) {
  var f = void 0;
  ...
  return f || create$1(v, dataset, deps);
}
...
var numericFilter = function numericFilter(v) {
  return typeof v === 'number' && !isNaN(v);
};
...
function minmax(settings, arr) {
  var definedMin = settings && typeof settings.min !== 'undefined';
  var definedMax = settings && typeof settings.max !== 'undefined';
  ...
  if (arr && arr.length) {
    if (!definedMin) {
      var arrMin = arr.map(minAccessor).filter(numericFilter);
      min = arrMin.length ? Math.min.apply(Math, toConsumableArray(arrMin)) : min;
    }

    if (!definedMax) {
      var arrMax = arr.map(maxAccessor).filter(numericFilter);
...
function ticksByValue(_ref4) {
  ...
  return values.sort(function (a, b) {
    ...
    return v <= scale.max() && v >= scale.min() && ary.indexOf(v) === i;
  }).map(function (tick) {
    return {
      position: scale(tick),
      label: formatter(tick),
...
function generateContinuousTicks(_ref5) {
  ...
  if (Array.isArray(settings.ticks.values)) {
    ...
    ticks = ticksByCount({ count: count, minorCount: minorCount, scale: scale.copy(), formatter: formatter });
  } else {
...
function continuousDefaultSettings() {
  return {
    ticks: {
      tight: false,
      forceBounds: false,
...
var AVAILABLE_SETTINGS = ['min', 'max', 'expand', 'include', 'invert'];
...
function generateSettings(settings, fields) {
  var calcSettings = {};
  AVAILABLE_SETTINGS.forEach(function (s) {
...
function scaleLinear$$1(settings, data) {
  var d3Scale = linear();
  ...
  fn.ticks = function ticks(input) {
    if (input !== null && (typeof input === 'undefined' ? 'undefined' : _typeof(input)) === 'object') {
      ...
      input.settings = index(true, continuousDefaultSettings(), settings, input.settings);
      input.scale = fn;
  ...
  fn.copy = function copy() {
    var cop = scaleLinear$$1(settings, data);
  ...
  if (settings) {
    var stgns = generateSettings(settings, data ? data.fields : []);
...
var AVAILABLE_SETTINGS$1 = ['padding', 'paddingOuter', 'paddingInner', 'align', 'invert', 'maxPxStep'];

function evalSetting$1(settings, fields, name) {
  if (typeof settings[name] === 'function') {
    return settings[name](fields);
  }
...
function generateSettings$1(settings, fields) {
  var calcSettings = {};
  AVAILABLE_SETTINGS$1.forEach(function (s) {
    calcSettings[s] = evalSetting$1(settings, fields, s);
  });
  return calcSettings;
}
...
function scaleBand() {
  ...
  var labelFn = typeof settings.label === 'function' ? settings.label : valueFn;
  ...
  var stgns = generateSettings$1(settings || {}, data.fields);
...
function scaleSequentialColor() {
  ...
  var s = scaleLinear$$1(settings, data, {}).clamp(true).interpolate(interpolateRgb);
  ...
  var _minmax = minmax(settings, data ? data.fields : []),
      _minmax2 = slicedToArray(_minmax, 2),
      min = _minmax2[0],
      max = _minmax2[1];

  var num = settings.domain ? settings.domain.length : -1;
  var DEFAULT_COLORS = theme ? theme.palette('sequential', num > 0 ? num : 2) : [];
  var range = typeof settings.range === 'function' ? settings.range(data, { theme: theme }) : settings.range || DEFAULT_COLORS;
...
function generateNiceDomain(range, min, max) {
  ...
  var lin = linear().domain([min, max]).nice([numPoints]);
...
function scaleThresholdColor() {
  ...
  var _minmax = minmax(settings, fields),
      _minmax2 = slicedToArray(_minmax, 2),
      min = _minmax2[0],
      max = _minmax2[1];

  var num = settings.domain ? settings.domain.length : -1;
  var DEFAULT_COLORS = theme.palette('sequential', num > 0 ? num : 2);
  var range = settings.range || DEFAULT_COLORS;
  var domain = settings.domain || (settings.nice ? generateNiceDomain(range, min, max) : [min + (max - min) / 2]);
...
function ordinal$1() {
  ...
  fn.label = function (domainValue) {
    var idx = values.indexOf(domainValue);
    return labels[idx];
  };
...
function scaleCategorical() {
  ...
  var s = ordinal$1(settings, dataset);
  ...
  if (settings.explicit && settings.explicit.domain) {
    ...
    var explicitDomain = settings.explicit.domain || [];
    if (explicitDomain.length) {
      ...
      var explicitRange = settings.explicit.range || [];
...
function create$2(options, d, deps) {
  ...
  var type = options.type || deduceScaleTypeFromOptions(options, data.fields);
  var s = void 0;
...
var EventEmitter = {
  ...
  mixin: function mixin(obj) {
    Object.keys(index$1.prototype).forEach(function (key) {
      obj[key] = index$1.prototype[key];
    });
    index$1.init(obj);
    return obj;
  }
...
function scrollApi() {
  ...
  var s = {
    ...
    getState: function getState() {
      return { min: min, max: max, start: start, viewSize: viewSize };
    }
  };
...
function index$2(boundaries, point, after) {
  var i = 0;
...
function rangeCollection() {
  ...
  fn.add = function (range) {
    ...
    var i0 = index$2(boundaries, min);
    var i1 = index$2(boundaries, max, true);
  ...
  fn.containsRange = function (range) {
    ...
    var i0 = index$2(boundaries, min, true);
    var i1 = index$2(boundaries, max);
...
function collectUnique(items) {
  ...
  for (var i = 0, num = items.length; i < num; i++) {
    ...
    var idx = filteredSet[key].indexOf(value);
...
function applyAliases(items, aliases) {
  ...
  var its = Array(len);
  for (var i = 0; i < len; i++) {
    its[i] = items[i].key in aliases ? index({}, items[i], { key: aliases[items[i].key] }) : items[i];
  }
  return its;
}
...
function resolveTapEvent(_ref5) {
  ...
  return resolveEvent({ collisions: collisions, t: t, config: config, action: resolveAction(t.action, e, 'toggle') });
}
...
function resolveOverEvent(_ref6) {
  ...
  return resolveEvent({ collisions: collisions, t: t, config: config, action: resolveAction(t.action, e, 'set') });
}

var isReservedProperty$1 = function isReservedProperty(prop) {
  return ['on', 'preferredSize', 'created', 'beforeMount', 'mounted', 'resize', 'beforeUpdate', 'updated', 'beforeRender', 'render', 'beforeDestroy', 'destroyed', 'defaultSettings', 'data', 'settings', 'formatter', 'scale', 'chart', 'dockConfig', 'mediator', 'style'].some(function (name) {
    return name === prop;
  });
};
...
function componentFactory(definition) {
  ...
  var emitter = EventEmitter.mixin({});
  var _settings = index(true, {}, defaultSettings, config);
  var _data = [];
  var _scale = void 0;
  ...
  var instanceContext = index({}, config, componentMixins.filter(function (mixinName) {
  ...
  var rend = rendString ? renderer || registries.renderer(rendString)() : renderer || registries.renderer()();
  ...
  fn.set = function () {
    ...
    if (opts.settings) {
      _settings = index(true, {}, defaultSettings, opts.settings);
      updateDockConfig(_dockConfig, _settings);
    }
  ...
  fn.getBrushedShapes = function getBrushedShapes(context, mode, props) {
    var shapes = [];
    if (_settings.brush && _settings.brush.trigger) {
      var brusher = _chart.brush(context);
      var sceneObjects = rend.findShapes('*');
      _settings.brush.trigger.forEach(function (b) {
        sceneObjects.forEach(function (sceneObject) {
          var nodeData = sceneObject.data;
  ...
  createClass(NarrowPhaseCollision, null, [{
    ...
    */
    ...
    value: function testCircleLine(circle, line) {
      ...
      var _getLineVectors = getLineVectors(line),
          _getLineVectors2 = slicedToArray(_getLineVectors, 2),
          p1 = _getLineVectors2[0],
          p2 = _getLineVectors2[1];
    ...
    */
    ...
    value: function testPolygonPoint(polygon, point) {
      ...
      var _getMinMax = getMinMax(polygon.vertices),
          _getMinMax2 = slicedToArray(_getMinMax, 1),
          xMin = _getMinMax2[0];

      var rayStart = { x: xMin - 1, y: point.y };
    ...
    */
    ...
    value: function testPolygonLine(polygon, line) {
      ...
      var _getLineVectors3 = getLineVectors(line),
          _getLineVectors4 = slicedToArray(_getLineVectors3, 2),
          p1 = _getLineVectors4[0],
          p2 = _getLineVectors4[1];
    ...
    */
    ...
    value: function testPolygonRect(polygon, rect) {
      ...
      var _getRectVertices = getRectVertices(rect),
          _getRectVertices2 = slicedToArray(_getRectVertices, 4),
          p1 = _getRectVertices2[0],
          p2 = _getRectVertices2[1],
          p3 = _getRectVertices2[2],
          p4 = _getRectVertices2[3];
    ...
    */
    ...
    value: function testRectLine(rect, line) {
      ...
      var _getLineVectors5 = getLineVectors(line),
          _getLineVectors6 = slicedToArray(_getLineVectors5, 2),
          p1 = _getLineVectors6[0],
          p2 = _getLineVectors6[1];

      if (NarrowPhaseCollision.testRectPoint(rect, p1) || NarrowPhaseCollision.testRectPoint(rect, p2)) {
        return true;
      }

      var rectEdges = getRectVertices(rect);
    ...
    */
    ...
    value: function testLineLine(line1, line2) {
      var _getLineVectors7 = getLineVectors(line1),
          _getLineVectors8 = slicedToArray(_getLineVectors7, 2),
          p1 = _getLineVectors8[0],
          p2 = _getLineVectors8[1];

      var _getLineVectors9 = getLineVectors(line2),
          _getLineVectors10 = slicedToArray(_getLineVectors9, 2),
          p3 = _getLineVectors10[0],
          p4 = _getLineVectors10[1];
    ...
    */
    ...
    value: function testLinePoint(line, point) {
      ...
      var _getLineVectors11 = getLineVectors(line),
          _getLineVectors12 = slicedToArray(_getLineVectors11, 2),
          p1 = _getLineVectors12[0],
          p2 = _getLineVectors12[1];
...
function res(style, references, path) {
  var refs = index({}, references, style);
  var s = {};
  var p = path.slice();
  Object.keys(style).forEach(function (key) {
    ...
    if (VARIABLE_RX.test(value) && value in refs) {
      ...
      if ((typeof value === 'undefined' ? 'undefined' : _typeof(value)) === 'object') {
        if (value[EXTEND]) {
          value = index({}, refs[value[EXTEND]], value);
        }
...
function themeFn() {
  ...
  palettes.forEach(function (palette) {
    pals[palette.key] = {
      ...
      sizes: palette.colors.map(function (colors) {
...
function addComponentDelta(shape, containerBounds, componentBounds) {
  ...
  var deltaShape = index(true, {}, shape);
...
function chart(definition, context) {
  ...
  var listeners = [];
  var instance = index({}, definition, chartMixins.filter(function (mixinName) {
  ...
  var createComponent = function createComponent(compSettings, container) {
    ...
    return {
      ...
      settings: index(true, {}, compSettings),
  ...
  var moveToPosition = function moveToPosition(comp, index$$1) {
    var el = comp.instance.renderer().element();
    if (isNaN(index$$1) || !el || !element || !element.childNodes) {
      return;
    }
    var nodes = element.childNodes;
    var i = Math.min(nodes.length - 1, Math.max(index$$1, 0));
    var node = nodes[i];
  ...
  var mount = function mount() {
    ...
    var onBrushTap = function onBrushTap(e) {
      ...
      for (var i = visibleComponents.length - 1; i >= 0; i--) {
        var comp = visibleComponents[i];
  ...
  instance.componentsFromPoint = function (p) {
    ...
    var tp = { x: p.x - br.left, y: p.y - br.top };
    var ret = [];
    visibleComponents.forEach(function (c) {
...
chart.mixin = add; // Expose mixin registering function
  ...
  for (var i = 0; i < arr.length; i++) {
    if (arr[i] === '*' && Array.isArray(container)) {
      ...
      for (var c = 0; c < container.length; c++) {
        carr.push.apply(carr, toConsumableArray(resolve$1(subpath, container[c])));
      }
      return carr;
      ...
      for (var _c = 0; _c < container.length; _c++) {
        _carr[_c] = resolve$1(subpath, container[_c]);
      }
      return _carr;
      // return container.map(v => resolve(arr.slice(i + 1).join('/'), v));
...
var reducers = {
  ...
  min: function min(values) {
    var filtered = filters$1.numeric(values);
...
function getPropsInfo(cfg, dataset) {
  // console.log('222', cfg);
  var _normalizeProperties = normalizeProperties(cfg, dataset, { main: { value: cfg.value, reduce: cfg.reduce } }, {}),
...
function extract$1(config, dataset) {
  ...
  cfgs.forEach(function (cfg) {
    if (typeof cfg.field !== 'undefined') {
      ...
      var trackFn = typeof cfg.trackBy === 'function' ? cfg.trackBy : null;
      var tracker = {};
      var trackedItems = [];
      var trackId = void 0;

      var items = f.items().map(function (v, idx) {
        var mainCell = v;
      ...
      if (track) {
        ...
        dataItems.push.apply(dataItems, toConsumableArray(items));
      }
    }
  });
  return dataItems;
...
function createFields(matrix, _ref) {
  var cache = _ref.cache;

  if (!matrix) {
    return;
  }
  var headers = matrix[0]; // assume headers are in first row TODO - add headers config

  var content = matrix.slice(1);

  headers.forEach(function (a, i) {
    var values = resolve$1('//' + i, content);
    var numericValues = filters.numeric(values);
...
function ds() {
  ...
  var dataset = {
     ...
     */
    extract: function extract(config) {
      return extract$1(config, dataset, cache);
    },
...
function polyFillPath2D$1(window) {
  if (!window) {
    return;
  }
  if (window.Path2D) {
   ...
   * @type {Object}
   */
  var argLength = { a: 7, c: 6, h: 1, l: 2, m: 2, q: 4, s: 4, t: 2, v: 1, z: 0 };

  /**
   * segment pattern
   * @type {RegExp}
   */
  var segmentPattern = /([astvzqmhlc])([^astvzqmhlc]*)/ig;
  ...
  function parse(path) {
    var data = [];
    path.replace(segmentPattern, function (_, command, args) {
  ...
  var _fill = window.CanvasRenderingContext2D.prototype.fill;
  var _stroke = window.CanvasRenderingContext2D.prototype.stroke;
...
if (typeof window !== 'undefined') {
  polyFillPath2D$1(window);
}

function doodler(settings) {
  function doodle() {
    doodle.settings = settings || { style: {} };
    return doodle;
  }

  doodle.style = function styleFn(object, styleName) {
    var style = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};

    return index(object, style[styleName] || {});
  };

  doodle.horizontalLine = function horizontalLine(x, y, width, styleName) {
  ...
  doodle.verticalLine = function verticalLine(x, y1, y2, styleName) {
    var style = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : {};
  ...
  doodle.whisker = function whisker(x, y) {
    var style = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : { whisker: {} };

    var width = style.whisker.width || 1;
    return doodle.style({
      type: 'line',
      y1: y,
      x1: x - width / 2,
  ...
  doodle.openwhisker = function openwhisker(x, y) {
    var style = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : { whisker: {} };

    var width = style.whisker.width || 1;
    return doodle.whisker(x - width / 2, y, style);
  };

  doodle.closewhisker = function closewhisker(x, y) {
    var style = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : { whisker: {} };
  ...
  doodle.box = function box(x, y, height) {
    var style = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : { box: {} };

    var width = style.box.width || 1;
    return doodle.style({
      type: 'rect',
      x: x - width / 2,
      y: y,
...
function normalizeSettings(settings, defaults$$1, chart) {
  var composition = index({}, settings);
  var defs = index({}, defaults$$1);
...
function resolveForItem(item, normalized, dataContext) {
  var ret = {};
  var keys = Object.keys(normalized);
  var len = keys.length;
  var fallbackData = item;
  for (var i = 0; i < len; i++) {
    var key = keys[i];
    var normalizedProp = normalized[key];
    var exists = typeof normalizedProp !== 'undefined';
    var hasExplicitDataProp = exists && typeof normalizedProp.ref === 'string';
    var hasImplicitDataProp = exists && key in item;
    var propData = hasExplicitDataProp ? item[normalizedProp.ref] : hasImplicitDataProp ? item[key] : fallbackData; // eslint-disable-line
    if (isPrimitive(normalizedProp)) {
      ...
      var idx = dataContext ? dataContext.indexOf(item) : -1;
      ret[key] = normalizedProp.fn.call(fnContext, hasExplicitDataProp && item ? item[normalizedProp.ref] : undefined, idx, dataContext);
...
function dispersion(chart) {
  var defaultStyles = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
  var initialSettings = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};

  var settings = initialSettings;
  var major = void 0;
  var minor = void 0;
  var bandwidth = void 0;
  var minDataPointDistance = void 0;
  var blueprint = void 0;
  var doodle = void 0;
  var items = void 0;
  var resolvedStyle = void 0;
  var majorNormalized = void 0;

  var fn = function fn() {
    return fn;
  };

  fn.updateSettings = function (stngs) {
  ...
  fn.onData = function (data, rect) {
    items = [];
    resolvedStyle = resolveInitialStyle(settings, defaultStyles, chart);

    if (major && major.pxScale) {
      var flipXY = settings.orientation === 'horizontal';
      var majorLenght = flipXY ? rect.height : rect.width;
      major = major.pxScale(majorLenght);
    }
    if (minor && minor.pxScale) {
      // is this needed or is minor never a band scale
      var _flipXY = settings.orientation === 'horizontal';
      var minorLenght = _flipXY ? rect.width : rect.height;
      minor = minor.pxScale(minorLenght);
    }

    // const bw = major && major.bandwidth ? major.bandwidth() : 0;

    var majorRef = settings.major ? settings.major.ref || '' : '';
    var majorStartRef = (typeof majorRef === 'undefined' ? 'undefined' : _typeof(majorRef)) === 'object' ? settings.major.ref.start : null;
    var majorEndRef = (typeof majorRef === 'undefined' ? 'undefined' : _typeof(majorRef)) === 'object' ? settings.major.ref.end : null;

    // Calculate the minimum data point distance
    if (major && !major.bandwidth && typeof majorRef === 'string') {
      var pointCoords = data.items.map(function (d) {
        return majorRef === '' ? d.value : d[majorRef].value;
      });

      // Sort values
      pointCoords.sort(function (a, b) {
        return a - b;
      });

      var minSpace = pointCoords[pointCoords.length - 1];
    ...
    data.items.forEach(function (d, i, all) {
      var obj = {};
      Object.keys(resolvedStyle).forEach(function (part) {
      ...
      if (it.majorStart !== null) {
        // if a majorstart/end are defined, calculate the midpoint
  ...
  fn.render = function (rect, buildShapes) {
    if (minDataPointDistance !== null) {
      if (minDataPointDistance === 0) {
        minDataPointDistance = 0.000000001;
      }
      var normalizedWidth = major(major.domain()[0]) - major(major.domain()[0] + minDataPointDistance);
    ...
    items.forEach(function (item) {
      var shapes = buildShapes({ item: item, blueprint: blueprint, doodle: doodle });
      shapes.forEach(function (shape) {
        shape.data = item.data;
        shape.collider = { type: null };
      });
      output.push.apply(output, toConsumableArray(shapes));
    });
...
var boxMarkerComponent = {
  ...
  buildShapes: function buildShapes(_ref2) {
    var item = _ref2.item,
        blueprint = _ref2.blueprint,
        doodle = _ref2.doodle;
    ...
    var bandwidth = this.dispersion.bandwidth();
    var span = item.majorEnd - item.majorStart;
    var majorStart = item.major;
    if (item.majorStart !== null && !isNaN(span)) {
      majorStart = item.majorStart + span * 0.5;
      bandwidth = Math.abs(span);
    }

    var majorStartModified = void 0;
    var majorEnd = void 0;
    var boxWidth = void 0;

    // Draw the box
    if (item.style.box.show && !notNumber(item.start) && !notNumber(item.end)) {
      var high = Math.max(item.start, item.end);
      var low = Math.min(item.start, item.end);
      ...
      shapes.push(blueprint.processItem({
        fn: function fn(_ref3) {
          ...
          var highModified = cap(-100, height + 200, Math.floor(high * height));
          var lowModified = cap(-100, height + 200, Math.floor(low * height));
          ...
          var wantedHeight = highModified - lowModified;
          var boxHeight = Math.max(item.style.box.minHeightPx, wantedHeight);
          var yModifier = (boxHeight - wantedHeight) / 2;

          return index(doodle.style({}, 'box', item.style), {
            type: 'rect',
    ...
    if (item.style.line.show && !notNumber(item.min) && !notNumber(item.start)) {
      ...
      shapes.push(blueprint.processItem({
        fn: function fn(_ref4) {
          ...
          return index(doodle.style({}, 'line', item.style), {
            type: 'line',
    ...
    if (item.style.median.show && !notNumber(item.med)) {
      shapes.push(blueprint.processItem({
        fn: function fn(_ref6) {
          ...
          return index(doodle.style({}, 'median', item.style), {
            type: 'line',
            y1: item.med * height,
    ...
    if (item.style.whisker.show && !notNumber(item.min) && !notNumber(item.max)) {
      // Low whisker
      var whiskerWidth = boxWidth * item.style.whisker.width;

      shapes.push(blueprint.processItem({
        fn: function fn(_ref7) {
          var height = _ref7.height;
          return index(doodle.style({ type: 'line' }, 'whisker', item.style), {
...
function getPointSizeLimits(x, y, width, height, limits) {
  ...
  return { min: min, max: max, maxGlobal: limits.maxPx, minGlobal: limits.minPx };
}

function calculateLocalSettings(stngs, chart) {
  var local = normalizeSettings(stngs, DEFAULT_DATA_SETTINGS, chart);
...
function createDisplayPoints(dataPoints, _ref, pointSize, shapeFn) {
  ...
  return dataPoints.filter(function (p) {
    ...
    var s = notNumber(p.size) ? p.errorShape : p;
...
var pointMarkerComponent = {
  ...
  render: function render(_ref2) {
    ...
    var limits = index({}, SIZE_LIMITS, this.settings.settings.sizeLimits);
    var points = (data.items || []).map(function (p, i, all) {
      var obj = resolveForItem(p, _this.local, all);
      obj.errorShape = resolveForItem(p, _this.local.errorShape, all);
      obj.data = p;
      return obj;
    });
    var pointSize = getPointSizeLimits(this.local.x, this.local.y, width, height, limits);
    return createDisplayPoints(points, this.rect, pointSize, this.settings.shapeFn || create$3);
  }
};

function pointMarker(picasso) {
  picasso.component('point-marker', pointMarkerComponent);
}
...
function createDisplayPies(arcData, _ref, slices) {
  var x = _ref.x,
      y = _ref.y,
      width = _ref.width,
      height = _ref.height;
...
var pieComponent = {
  ...
  updateSettings: function updateSettings(settings) {
    var chart = this.chart;
    this.local = normalizeSettings(settings.settings.slice, DEFAULT_DATA_SETTINGS$1, chart);
  },
  beforeUpdate: function beforeUpdate(settings) {
    this.updateSettings(settings);
  },
  beforeRender: function beforeRender(opts) {
    this.rect = opts.size;
  ...
  render: function render(_ref2) {
    ...
    var slices = (data.items || []).map(function (s, i, all) {
      var obj = resolveForItem(s, _this.local, all);
      obj.data = s;
...
function discreteDefaultSettings() {
  return {
    labels: {
      show: true,
      tiltAngle: 40,
...
function continuousDefaultSettings$1() {
  return {
    labels: {
      show: true,
      tiltAngle: 40,
...
var gridLineComponent = {
  ...
  render: function render() {
    ...
    var axisDefaults = continuousDefaultSettings$1();
    this.settings.ticks = index({}, axisDefaults.ticks, this.settings.ticks || {});
    this.settings.minorTicks = index({}, axisDefaults.minorTicks, this.settings.minorTicks || {});
...
function createLineWithLabel(_ref) {
  ...
  var label = false;
  var value = false;
  var style = index(true, {}, settings.style.line, p.line || {});
  ...
  if (p.label) {
    var item = index(true, refLabelDefaultSettings(), settings.style.label || {}, { fill: style.stroke }, p.label);
  ...
  render: function render() {
    ...
    this.lines.x = this.lines.x.filter(filterUndefinedValue).map(function (line) {
      if (line.scale) {
        ...
        return index(line, { scale: scale, position: scale(line.value) });
      }

      return index(line, { position: line.value });
    });
    // Set all Y lines to flipXY by default
    // This makes the transposer flip them individually
    this.lines.y = this.lines.y.filter(filterUndefinedValue).map(function (line) {
      if (line.scale) {
        var scale = _this.chart.scale(line.scale);
        return index(line, { scale: scale, position: scale(line.value), flipXY: true });
      }

      return index(line, { position: line.value, flipXY: true });
    });
    ...
    [].concat(toConsumableArray(this.lines.x), toConsumableArray(this.lines.y)).forEach(function (p) {
      ...
      if (show) {
        ...
        createLineWithLabel({ chart: _this.chart, blueprint: _this.blueprint, renderer: _this.renderer, p: p, settings: settings, items: items });
      }
    });

    // Handle out of bounds
    if (settings.style.oob.show) {
      oobManager({ blueprint: this.blueprint, oob: oob, settings: settings, items: items });
    }
...
function adjustForEnds(struct, buildOpts) {
  ...
  if (buildOpts.align === 'top' || buildOpts.align === 'bottom') {
    ...
    if (topTextBoundary < topBoundary) {
      ...
      var alphabeticalBaselineDivisor = 3;
...
function adjustForEnds$1(struct, buildOpts) {
  var halfWidth = struct.strokeWidth / 2;
...
function buildNode$1(tick, buildOpts) {
  ...
  adjustForEnds$1(struct, buildOpts);
  return struct;
}
...
function getSize(_ref7) {
  ...
  var size = 0;
  var edgeBleed = { left: 0, top: 0, right: 0, bottom: 0 };
  ...
  if (settings.labels.show) {
    ...
    if (isDiscrete && horizontal && settings.labels.mode === 'auto') {
      if (shouldAutoTilt({ majorTicks: majorTicks, measure: measure, rect: rect, scale: scale, state: state, settings: settings })) {
    ...
    if (isDiscrete && state.labels.activeMode !== 'tilted' && isToLarge({ rect: rect, scale: scale, state: state, majorTicks: majorTicks, measure: measure, horizontal: horizontal })) {
    ...
    if (state.labels.activeMode === 'tilted') {
      ...
      if (isDiscrete && isTiltedLabelOverlapping({ majorTicks: majorTicks, measureText: measureText, rect: rect, bleedSize: bleedSize, angle: settings.labels.tiltAngle })) {
...
function layeredLabelBuilder(ticks, buildOpts, settings, measureText) {
  ...
  var padding2 = labelsSpacing(settings) + buildOpts.maxHeight + settings.labels.margin;
  return ticks.map(function (tick, i) {
    buildOpts.layer = i % 2;
...
function nodeBuilder(isDiscrete) {
  ...
  function discrete() {
    ...
    getStepSizeFn = function getStepSizeFn(_ref4) {
  ...
  function build(_ref5) {
    ...
    if (settings.labels.show) {
      var textRect = calcMaxTextRectFn({ measureText: measureText, settings: settings, innerRect: innerRect, ticks: ticks, scale: scale, tilted: tilted, layered: layered });
      var padding = labelsSpacing(settings);
      buildOpts.stepSize = getStepSizeFn({ innerRect: innerRect, scale: scale, ticks: ticks, settings: settings });
...
function wrapper(globalFallback, fallbackVal, fn, fnContext) {
  for (var _len = arguments.length, args = Array(_len > 4 ? _len - 4 : 0), _key = 4; _key < _len; _key++) {
    args[_key - 4] = arguments[_key];
  }

  var value = fn ? fn.call.apply(fn, [fnContext].concat(args)) : null;
  if (typeof value !== 'undefined') {
  ...
  if (fallbackVal && typeof fallbackVal.fn === 'function') {
    ...
    return fallbackVal.fn.apply(fallbackVal, [fnContext].concat(args));
  }
  // fallback is a value, return it
  return fallbackVal;
}

function attr(targets, attribute, defaultVal, index$$1) {
  var target = targets[index$$1];
  var globalDefault = GLOBAL_DEFAULTS$1[attribute];
  if (!target) {
    return defaultVal;
  }
  var type = _typeof(target[attribute]);

  if (type === 'undefined') {
    // undefined value
    if (index$$1 < targets.length - 1) {
      // check inheritance
      return attr(targets, attribute, defaultVal, index$$1 + 1);
    }
    // end of the chain, return default
    return defaultVal;
  } else if (_typeof(target[attribute]) === (typeof defaultVal === 'undefined' ? 'undefined' : _typeof(defaultVal)) || target[attribute] === null) {
  ...
  if (type === 'function') {
    ...
    var inner = attr(targets, attribute, defaultVal, index$$1 + 1);
    target[attribute].fn = function () {
      for (var _len2 = arguments.length, args = Array(_len2), _key2 = 0; _key2 < _len2; _key2++) {
        args[_key2] = arguments[_key2];
      }

      return wrapper.apply(undefined, [globalDefault, inner, target[attribute]].concat(args));
    };
...
*    "parts.rect" );
*/
function resolveStyle(defaults$$1, styleRoot, path) {
  var steps = path ? path.split('.') : [];
  var ret = {};
  Object.keys(defaults$$1).forEach(function (s) {
    var def = defaults$$1[s] === null && typeof GLOBAL_DEFAULTS$1[s] !== 'undefined' ? GLOBAL_DEFAULTS$1[s] : defaults$$1[s];
...
*/
function resolveForDataValues(styles, dataValues, index$$1) {
  var ret = {};
  if (dataValues) {
    Object.keys(styles).forEach(function (s) {
      ret[s] = styles[s] && typeof styles[s].fn === 'function' ? styles[s].fn(undefined, dataValues[s][index$$1], index$$1, dataValues[s]) : styles[s];
    });
  } else {
    Object.keys(styles).forEach(function (s) {
...
function isPrimitive$2(v) {
  return (typeof v === 'undefined' ? 'undefined' : _typeof(v)) !== 'object';
}

function resolveForDataObject(props, dataObj, index$$1, allData, contextProps) {
  var ret = {};
  Object.keys(props).forEach(function (s) {
    var exists = typeof props[s] !== 'undefined';
    var hasScale = exists && typeof props[s].scale === 'function';
    var hasExplicitDataProp = exists && !!props[s].ref;
    // const hasImplicitDataProp = typeof props[s] === 'object' ? s in dataObj : false;
    var propData = exists && props[s].ref ? dataObj[props[s].ref] : dataObj;
    if (typeof props[s] === 'function') {
      // custom accessor function, not scale!
      var fnContext = index({}, { data: dataObj }, contextProps);
      if (hasScale) {
        fnContext.scale = props[s].scale;
      }
      if (typeof props[s].fn === 'function') {
        ret[s] = props[s].fn(fnContext, hasExplicitDataProp ? dataObj[props[s].ref] : undefined, index$$1, allData);
      } else {
        ret[s] = props[s].call(fnContext, hasExplicitDataProp ? dataObj[props[s].ref] : undefined, index$$1, allData);
      }
    } else if (hasScale) {
      // } && (hasImplicitDataProp || hasExplicitDataProp)) {
      ret[s] = props[s].scale(isPrimitive$2(propData) ? propData : propData.value);
...
function resolveSettingsForPath(settings, defaults$$1, chart, path) {
  var composition = index(true, {}, settings);
  // Check each setting for the existance of a source without custom function
  var ext = path ? composition[path] : composition;
  var defs = path ? defaults$$1[path] : defaults$$1;
  if (ext) {
    Object.keys(ext).forEach(function (a) {
      if (ext[a]) {
        if ((ext[a].source || ext[a].scale) && !ext[a].fn) {
          var scale = chart.scale(ext[a]);
          if (scale) {
            ext[a] = scale;
          }
        } else if (ext[a].fn) {
...
function resolveInitialStyle$1(settings, baseStyles, chart) {
  var ret = {};
  Object.keys(baseStyles).forEach(function (s) {
...
var axisComponent = {
  ...
  created: function created() {
    ...
    if (this.state.isDiscrete) {
      ...
      this.state.defaultStyleSettings = continuousDefaultSettings$1();
      this.state.defaultDock = 'left';
    }

    this.setState(this.settings);
  },
  setState: function setState(settings) {
    var _this = this;

    this.state.isDiscrete = !!this.scale.bandwidth;
    var styleSettings = resolveInitialStyle$1(settings.settings, this.state.defaultStyleSettings, this.chart);
    this.state.settings = index(true, {}, this.settings, settings.settings, styleSettings);

    var dock = typeof this.state.settings.dock !== 'undefined' ? this.state.settings.dock : this.state.defaultDock;
    ...
    Object.keys(styleSettings).forEach(function (a) {
      _this.state.settings[a] = resolveForDataValues(_this.state.settings[a]);
    });
  ...
  resize: function resize(opts) {
    ...
    var extendedInner = index({}, inner, alignTransform({
      align: this.state.settings.align,
      inner: inner
    }));

    var finalOuter = outer || extendedInner;
    index(this.state.innerRect, extendedInner);
    index(this.state.outerRect, finalOuter);
...
function generateTitle(_ref) {
  ...
  index(struct, definitionSettings.style);
...
var textComponent = {
  ...
  beforeUpdate: function beforeUpdate(opts) {
    if (opts.settings) {
      index(this.settings, opts.settings);
...
function startArea(_ref) {
  ...
  if (activeRange.mode !== 'modify' && state.targetRect && !NarrowPhaseCollision.testRectPoint(state.targetRect, { x: relX, y: relY })) {
    // do nothing
  } else {
...
function start$1(_ref2) {
  ...
  state.offset = index({}, renderer.element().getBoundingClientRect());
  var relX = x - state.offset.left; // coordinate relative renderer
  var relY = y - state.offset.top;
...
function ranges(state, brush$$2) {
  if (!brush$$2 || !brush$$2.isActive()) {
    return [];
  }
  ...
  var rangeBrush = brush$$2.brushes().filter(function (f) {
...
function resolveNodeBounds(targetNodes) {
  var points = targetNodes.reduce(function (ary, node) {
    ary.push.apply(ary, toConsumableArray(getRectVertices(node.bounds)));
    return ary;
  }, []);
  return pointsToRect(points);
}
...
function getBubbleLabel(state, value, range) {
  ...
  var dataConfig = compConfig.data || ['self'];
...
function placeTextInRect(rect, text, opts) {
  ...
  var wiggleHeight = Math.max(0, rect.height - textMetrics.height * LINE_HEIGHT);
...
function placeVerticalTextInRect(rect, text, opts) {
  var label = {
    type: 'text',
    text: text,
  ...
  var wiggleHor = Math.max(0, rect.width - textMetrics.height * LINE_HEIGHT);
...
function placeInVerticalBars(_ref2) {
  var nodes = _ref2.nodes,
      stngs = _ref2.stngs,
      placementSettings = _ref2.placementSettings,
      labelSettings = _ref2.labelSettings,
  ...
  var labels = [];
  var textPlacementFn = fitsHorizontally ? placeTextInRect : placeVerticalTextInRect;
  var label = void 0;
  var node = void 0;
  var d = void 0;
  var text = void 0;
  ...
  for (var i = 0, len = nodes.length; i < len; i++) {
    ...
    for (var j = 0; j < nodeTexts.length; j++) {
      text = nodeTexts[j];
      if (!text) {
      ...
      for (p = 0; p < placements.length; p++) {
        ...
        if (fitsHorizontally && testBounds.height > measured.height * LINE_HEIGHT || !fitsHorizontally && testBounds.height > measured.width) {
          bounds = testBounds;
          break;
        }
      }
      ...
      if (bounds && placement) {
        ...
        if (typeof placement.fill === 'function') {
          fill = placement.fill({ node: node, data: d }, i);
        } else {
          fill = placement.fill;
        }
  ...
  render: function render() {
    ...
    (stngs.sources || []).forEach(function (source) {
      if (source.strategy && source.strategy.type === 'bar' && source.component) {
        comp = _this.chart.component(source.component);
        if (comp) {
          nodes = _this.chart.findShapes(source.selector).filter(function (n) {
            return n.key === source.component;
          });
          nodeData = comp.data;
        }

        labels.push.apply(labels, toConsumableArray(bars({
...
function resolveMargin(margin) {
  if ((typeof margin === 'undefined' ? 'undefined' : _typeof(margin)) === 'object' && margin !== null) {
    var output = {};
    ...
    var parts = margin.replace(/px/gi, '').split(' ');
...
function directionTriangle$1(x, y, r) {
  var d = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 'bottom';
...
function createButton(_ref) {
  ...
  var r = Math.min(width / 2, height / 2) * 0.6;
  var symX = x + width / 2;
  var symY = y + height / 2;

  var buttonSymbol = index({}, symbol, {
    type: 'path',
    d: directionTriangle$1(symX, symY, r, direction)
  });
...
function categoricalLegend(picasso) {
  picasso.component('legend-cat', categoricalLegend$1);
}
...
var lineMarkerComponent = {
  ...
  render: function render(_ref2) {
    ...
    var orientation = this.stngs.orientation || 'horizontal';
    var pointSettings = normalizeSettings(this.stngs.coordinates, SETTINGS.coordinates, this.chart);
    var lineSettings = normalizeSettings(this.stngs.layers.line, SETTINGS.layers.line, this.chart);
    var areaSettings = normalizeSettings(this.stngs.layers.area, SETTINGS.layers.area, this.chart);
    var layerSettings = normalizeSettings({
      curve: this.stngs.layers.curve,
      show: this.stngs.layers.show
    }, { curve: SETTINGS.layers.curve, show: SETTINGS.layers.show }, this.chart);

    var missingMinor0 = typeof this.stngs.coordinates.minor0 === 'undefined';
    ...
    for (var i = 0; i < data.items.length; i++) {
      var p = data.items[i];
      var lid = resolveForItem(p, { layerId: pointSettings.layerId }).layerId;
      layerIds[lid] = layerIds[lid] || { order: numLines++, id: lid, items: [], firstPoint: p };
      layerIds[lid].items.push(p);
    }

    var layerData = Object.keys(layerIds).map(function (lid) {
      return layerIds[lid].firstPoint;
    });

    // massage layers
    var layers = Object.keys(layerIds).map(function (lid) {
      var layer = layerIds[lid];
      var layerObj = resolveForItem(layer.firstPoint, layerSettings, layerData);
      if (layerObj && layerObj.show === false) {
        return null;
      }

      var lineObj = resolveForItem(layer.firstPoint, lineSettings, layerData);
      var areaObj = resolveForItem(layer.firstPoint, areaSettings, layerData);

      var values = [];
      var points = [];
      var items = layer.items.slice();
      var point = void 0;
      var pData = void 0;
      for (var _i = 0; _i < layer.items.length; _i++) {
        pData = layer.items[_i];
    ...
    layers.forEach(function (layer) {
      ...
      if (orientation === 'vertical') {
        areaGenerator.y(function (d) {
          return d.major * height;
        }).x1(function (d) {
          return d.minor * width;
        }).x0(function (d) {
          return d.minor0 * width;
        }).defined(function (d) {
        ...
        areaGenerator.x(function (d) {
          return d.major * width;
        }).y1(function (d) {
          return d.minor * height;
        }).y0(function (d) {
          return d.minor0 * height;
        }).defined(function (d) {
          return typeof d.minor === 'number' && !isNaN(d.minor);
      ...
      // area layer
      if (_this.stngs.layers.area && areaObj.show !== false) {
        var areaPath = areaGenerator(points);
        nodes.push({
          type: 'path',
          d: areaPath,
      ...
      if (lineObj && lineObj.show !== false) {
        var linePath = lineGenerator(points);
        nodes.push({
          type: 'path',
          d: linePath,
        ...
        if (!missingMinor0 && _this.stngs.layers.area && areaObj.show !== false) {
...
function pointMarker$1(picasso) {
  picasso.component('line', lineMarkerComponent);
}

var components = [boxMarker, pointMarker, pie$1, gridLine, refLine, axis, text, scrollbar, rangeBrush, rangeBrush$1, lassoBrush, labels, categoricalLegend, sequentialLegend, pointMarker$1];
...
var GeoRect = function () {
  function GeoRect() {
    ...
    this.set({ x: x, y: y, width: width, height: height, minWidth: minWidth, minHeight: minHeight });
  }
...
var GeoCircle = function () {
  function GeoCircle() {
    ...
    this.set({ cx: cx, cy: cy, r: r, minRadius: minRadius });
  }
...
var GeoLine = function () {
  function GeoLine() {
    ...
    this.set({ x1: x1, y1: y1, x2: x2, y2: y2, tolerance: tolerance });
  }
  ...
  createClass(GeoLine, [{
    ...
    key: 'intersectsRect',
    value: function intersectsRect(vertices) {
      var rect = pointsToRect(vertices);
...
var GeoPolygon = function () {
  ...
  createClass(GeoPolygon, [{
    ...
    key: 'intersectsPolygon',
    value: function intersectsPolygon() {
      // eslint-disable-line
      return false;
    }
    ...
    value: function bounds() {
      if (!this._bounds) {
        ...
        var _minMax = slicedToArray(this._minMax, 4),
            xMin = _minMax[0],
            yMin = _minMax[1],
            xMax = _minMax[2],
            yMax = _minMax[3];

        this._bounds = [{ x: xMin, y: yMin }, { x: xMax, y: yMin }, { x: xMax, y: yMax }, { x: xMin, y: yMax }];
      }
...
reg$5.add('rect', create$8);
reg$5.add('circle', create$9);
reg$5.add('line', create$10);
reg$5.add('polygon', create$11);

function create$7(type, input) {
  // eslint-disable-line import/prefer-default-export
  return reg$5.get(type)(input);
}
...
function colliderToShape(node, dpi) {
  ...
  if (collider && collider.fn) {
    ...
    if (type === 'rect' || type === 'bounds') {
      ...
      var circle = pointsToCircle(points, collider.fn.r);
      circle.type = 'circle';
      return circle;
    } else if (type === 'line') {
      var line = pointsToLine(points);
...
function create$12() {
  ...
  return new (Function.prototype.bind.apply(SceneObject, [null].concat(a)))();
}
...
function resolveBoundsCollision(node, type, input) {
  ...
  if (Array.isArray(input.vertices)) {
    transformedInput = create$11(input); // TODO Shouldn't have to do this here, currently its beacause a collision algorithm optimization, i.e. caching of polygon bounds
  }

  if (collider[type](transformedInput)) {
    var c = create$13(node);
...
function resolveGeometryCollision(node, type, input) {
  ...
  if (Array.isArray(transformedInput.vertices)) {
    transformedInput = create$11(transformedInput); // TODO Shouldn't have to do this here, currently its beacause a collision algorithm optimization, i.e. caching of polygon bounds
  }
...
function resolveShape(shape) {
  ...
  if (isNumber(cx) && isNumber(cy) && isNumber(r)) {
    ...
    _shape = getLineVectors(shape).map(function (p) {
      return scalarMultiply(p, ratio);
    });
    return ['intersectsLine', _shape];
  } else if (isNumber(x) && isNumber(y) && isNumber(width) && isNumber(height)) {
    _shape = getRectVertices(shape).map(function (p) {
      return scalarMultiply(p, ratio);
    });
    ...
    _shape.vertices = vertices.map(function (vertex) {
...
var Container = function (_DisplayObject) {
  ...
  createClass(Container, [{
    ...
    value: function boundingRect() {
      ...
      return index({ x: 0, y: 0, width: 0, height: 0 }, this._boundingRect);
    }
  }, {
    key: 'bounds',
    value: function bounds() {
    ...
    value: function addChild(c) {
      ...
      if (this._collider && this._collider.type === 'bounds') {
        ...
        var opts = index({ type: 'bounds', x: 0, y: 0, width: 0, height: 0 }, this._boundingRect);
        get(Container.prototype.__proto__ || Object.getPrototypeOf(Container.prototype), 'collider', this).call(this, opts);
      }
    ...
    value: function removeChild(c) {
      ...
      if (this._collider && this._collider.type === 'bounds') {
        var opts = index(this.boundingRect(true), { type: 'bounds' });
        get(Container.prototype.__proto__ || Object.getPrototypeOf(Container.prototype), 'collider', this).call(this, opts);
      }
...
var Rect = function (_DisplayObject) {
  ...
  createClass(Rect, [{
    ...
    value: function set$$1() {
      ...
      var opts = index({ type: 'rect', x: x, y: y, width: width, height: height }, collider);
    ...
    value: function boundingRect() {
      ...
      var p = getRectVertices(this.attrs);
...
var Circle = function (_DisplayObject) {
  ...
  createClass(Circle, [{
    ...
    value: function set$$1() {
      ...
      var opts = index({ type: 'circle', cx: cx, cy: cy, r: r }, collider);
...
var Line = function (_DisplayObject) {
  ...
  createClass(Line, [{
    ...
    value: function set$$1() {
      ...
      get(Line.prototype.__proto__ || Object.getPrototypeOf(Line.prototype), 'collider', this).call(this, index(defaultCollider, collider));
    }
  }, {
    key: 'boundingRect',
    value: function boundingRect() {
      ...
      var p = getLineVectors(this.attrs);
...
function pathToPoints(path) {
  ...
  var commands = path.match(/[a-z][^a-z]*/ig);

  var prevPoint = void 0;
  var points = [];
  for (var i = 0; i < commands.length; i++) {
    var params = commands[i].match(/([mlz])\s?(\S+)?(?:\s|,)?(\S+)?/i);
...
var Text = function (_DisplayObject) {
  ...
  createClass(Text, [{
    ...
    value: function boundingRect() {
      ...
      var p = getRectVertices(this._boundingRect);
      var pt = includeTransform && this.modelViewMatrix ? this.modelViewMatrix.transformPoints(p) : p;
...
reg$4.add('rect', create$15);
reg$4.add('circle', create$16);
reg$4.add('text', create$19);
reg$4.add('line', create$17);
reg$4.add('path', create$18);
reg$4.add('stage', create$5);
reg$4.add('container', create$6);
reg$4.add('defs', create$14);
reg$4.add('linearGradient', create$14);
reg$4.add('radialGradient', create$14);
reg$4.add('stop', create$14);

function create$4(type, input) {
  // eslint-disable-line import/prefer-default-export
...
function traverse(items, parent, matrix, on) {
  ...
  for (var i = 0, len = items.length; i < len; i++) {
    ...
    var displayNode = create$4(state.node.type, state.node);
    if (displayNode) {
...
var widthMeasureCache = {};
var canvasCache = void 0;
...
function renderShapes(shapes, g, shapeToCanvasMap) {
  for (var i = 0, len = shapes.length; i < len; i++) {
    ...
    if (reg$3.has(shape.type)) {
      reg$3.get(shape.type)(shape.attrs, {
        g: g,
...
function renderer$1() {
  ...
  canvasRenderer.itemsAt = function (options) {
    return scene$$1 ? scene$$1.getItemsFrom(options) : [];
  };

  canvasRenderer.findShapes = function (selector) {
    if (scene$$1) {
      return scene$$1.findShapes(selector).map(function (s) {
   ...
   * }); // returns { width: 20, height: 12 }
   */
  canvasRenderer.measureText = function (_ref) {
    var text = _ref.text,
        fontSize = _ref.fontSize,
        fontFamily = _ref.fontFamily;
    return measureText({ text: text, fontSize: fontSize, fontFamily: fontFamily });
  };
  ...
  * @return {object} The bounding rectangle
  */
  canvasRenderer.textBounds = function (node) {
...
function register$1(type, renderFn) {
  reg$3.add(type, renderFn);
}

function render$3(rect, _ref) {
  var g = _ref.g,
      doFill = _ref.doFill,
...
register$1('rect', render$3);
register$1('circle', render$4);
register$1('line', render$5);
register$1('path', render$7);
register$1('text', render$6);

function rendererComponent(picasso) {
  picasso.renderer('canvas', renderer$1);
}
...
function renderer$2() {
  ...
  svg.render = function (nodes) {
    ...
    var newScene = sceneFn({
      items: [sceneContainer],
      on: {
        create: [onGradient, onLineBreak(measureText)]
      }
    });
    var hasChangedScene = scene$$1 ? !newScene.equals(scene$$1) : true;
...
function rendererComponent$1(picasso) {
  picasso.renderer('svg', renderer$2);
}
...
var patch = snabbdom.init([attributes, _class, style, eventlisteners]);
...
function renderer$4() {
  ...
  dom.itemsAt = function () {
    return [];
  };

  dom.findShapes = function () {
    return [];
  };
  ...
  dom.render = function (nodes) {
    ...
    var node = h('div', {}, Array.isArray(nodes) ? nodes : [nodes]);
...
function rendererComponent$2(picasso) {
  picasso.renderer('dom', renderer$4);
}
...
function pic() {
  ...
  var logger$$1 = logger(config.logger);
  var regis = {
    // -- registries --
  ...
  function picassojs() {
    ...
    var cc = index({ palettes: [] }, config, cfg);
